...
There's an annoying gap in the message broker world. On one end you have Redis pub/sub and MQTT QoS 0, which are fast and simple, but purely fire-and-forget. No delivery guarantees, no way to know if anything actually received your message. On the other end you have RabbitMQ, Kafka, and friends which are reliable, but 50 MB+ binaries that want a server, a config file, and ideally a dedicated ops person.

When building something at the edge, latency and memory-footprint size both plays a critical role. An ESP32 has 4 MB of flash. A sensor node on a flaky network needs to know its readings got through. You don't want to stand up a Kafka cluster for that.
...
- [ESP32/Arduino Support](#esp32arduino-support)
...
MetricMQ has built-in **Ed25519 message signing** for authentication and integrity verification.
...
| Signing format inconsistency | ✅ Fixed (Phase 1) | `crypto_demo.cpp` now signs `topic + payload` (no colon) — matches broker and ESP32 library |
...
Pull requests are welcome. The codebase is small enough that getting oriented doesn't take long. These are the areas that would have the most real-world impact right now:

1. **Manual ACK tests** — `auto_ack=false` is completely untested and probably the most dangerous gap
2. **RESP authentication** — right now any TCP client on port 6379 gets full access; even a simple password challenge would help
3. **Language bindings** — a Python or Go client would make this much more accessible
4. **TLS/DTLS** — Ed25519 handles message integrity, but connection-level encryption is still an open problem
